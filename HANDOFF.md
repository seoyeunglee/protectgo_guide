...
- **홈 재설계** (디자인 레퍼런스: 헬프센터 균일 그리드 카드 + 썸네일, NN/g 카드 패턴,
  GitBook IA): ① "가이드 이용 방법" 3단계 다이어그램(기능 선택 → 문서 유형 → 절차=핀 번호),
  ② 기능별 카드 — 실제 화면 캡처 썸네일 + 한 줄 설명 + 대표 링크, ③ 문서 유형 안내
  (Diátaxis 4유형 독자 언어 설명). 역할별 카드는 간소화 원칙으로 제거.
- **사이드바**: 도메인 × 유형 트리 → **기능(도메인) 그룹 드롭다운**(details/summary,
  현재 그룹 자동 펼침) + 항목별 유형 배지. 유형 인덱스 페이지는 경로 호환용으로만 생성.
- **콘텐츠 재배치 (삭제 아닌 이동)**:
  - "이 가이드가 만들어지는 방식"(sourcing) — 작성자용 내용이라 가이드에서 제거,
    `guide/README.md` "출처와 반영 흐름" 절로 이동. "예고된 개편의 승격" 절은 삭제(지시).
  - 개념 이해 2개(scenario-node-concepts, project-workspace-concepts) — 도메인 랜딩
    (`kb/<category>/README.md`)에 병합. 상위 개념 = 도메인 원칙.
  - KB 엔트리 8개 + 도메인 랜딩 2개, 가이드 19페이지. 끊긴 교차 링크 모두 수정.
- **CRUD·권한 분리 계획 도출** — 아래 "계획" 절 신설 (V2 설계 입력: 열람자/작성자/검토자,
  draft 승격·게시는 검토자 승인 게이트).
- 루트 README에 실행 방법(퀵스타트) 추가. PRD(Notion·미러) 구현 현황 IA 개편 반영.
...
## 계획 — 출처·반영 흐름의 사용자 입력/검토 CRUD와 권한 분리 (V2 설계 입력)

출처와 반영 흐름에서 사람 입력·검토가 필요한 지점을 CRUD로 도출한 것. 현재는 정적
사이트 + git 저장소 권한으로만 분리되며, 사이트 내부 권한 분리(작성·검토 UI)는 V2에서
에이전트 4단계의 publish 게이트와 함께 설계한다.

| 대상 | C | R | U | D | 비고 |
|---|---|---|---|---|---|
| KB 엔트리 (본문·frontmatter) | 작성자 | 모두 | 작성자 | 작성자 | 변경은 검토자 승인 후 게시 |
| provenance 메타 (version·captured_commit·deployment) | 도구 | 작성자·검토자 | 도구 실행(작성자) | — | 스크립트가 라인 단위 갱신, 사람이 결과 검토 |
| 화면 캡처·스냅샷 | 작성자(도구) | 모두 | 작성자(도구) | 작성자 | 캡처는 실제 솔루션만, draft 캡처 금지 규칙 유지 |
| draft → 확정 승격 | — | 검토자 | **검토자 승인** | — | 승격 게이트 — 작성자 단독 불가(권한 분리 핵심) |
| 점검 리포트 (check/fetch 결과) | 도구 | 작성자·검토자 | — | — | 검토 필요 항목의 처리 책임은 작성자 |
| 가이드 게시 (빌드·배포) | 작성자 | 열람자 | 검토자 승인 후 | — | 현재 수동 빌드, V2에서 승인 후 게시 흐름 |

역할(안): **열람자**(가이드 사용자 — 읽기 전용) / **작성자**(KB 편집·캡처·빌드 실행) /
**검토자**(draft 승격·게시 승인 — 기획·도메인 오너). 제품 권한 체계(에디터/매니저/뷰어)와
별개인 가이드 운영 권한이다.
